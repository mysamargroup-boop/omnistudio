...
async def list_image_models(request: Request):
    ...
    return {
        "models": [
            ...
            {"id": "imagen_3", "name": "Google Imagen 3", "provider": "google", "badge": "PRO", "active": google_active, "description": "Google's flagship hyper-realistic lighting & micro-textures"},
            {"id": "gemini_flash_image", "name": "Google Gemini 2.5 Flash", "provider": "google", "badge": "FAST", "active": google_active, "description": "Ultra-fast high fidelity image diffusion"},
...
def _sync_edit_image(src_file: Path, target_name: str, req: ImageEditRequest) -> dict:
...
async def edit_image(req: ImageEditRequest, request: Request):
    ...
    try:
        return await asyncio.to_thread(_sync_edit_image, src_file, target_name, req)
